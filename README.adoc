...
**Precision Time Protocol** — IEEE 1588 PTP support with sub-microsecond
synchronisation across nodes. Automatic clock source detection with graceful
fallback (PTP hardware → phc2sys → NTP → system). Built for multi-server
deployments where timing matters.
...
----
just test          # All tests
just test-server   # Elixir tests (49 tests)
